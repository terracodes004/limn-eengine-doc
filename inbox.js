...
async function loadInbox() {
  ...
  try {
    ...
    if (!engineUpdates || engineUpdates.length === 0) {
      container.innerHTML = `
        <div style="background: var(--surface); border: 1.5px solid var(--border); border-radius: 14px; padding: 30px; text-align: center;">
          <p style="color: var(--muted); font-weight: 500;">📭 No changelogs available yet. Generate one from the home page!</p>
        </div>
      `;
      return;
    }

    let readIds = JSON.parse(localStorage.getItem('limn_read_changelogs') || '[]');
    container.innerHTML = '';

    engineUpdates.forEach(item => {
      ...
      card.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; border-bottom: 1px solid rgba(255, 99, 140, 0.1); padding-bottom: 10px;">
          <h3 style="margin: 0; font-size: 1.1rem; font-weight: 700; color: #ffffff; display: flex; align-items: center;">
            ${unreadDot}${escapeHtml(item.title || 'Limn Engine Update')}
          </h3>
      ...
      if (!isRead) {
        card.addEventListener('click', () => {
          ...
          localStorage.setItem('limn_read_changelogs', JSON.stringify(readIds));
    ...
    container.innerHTML = `
      <div style="background: var(--surface); border: 1.5px solid #ef4444; border-radius: 14px; padding: 20px; text-align: center;">
        <p style="color: #ef4444; font-weight: 600;">⚠️ Failed to load inbox messages.</p>
      </div>
    `;
  }
}
